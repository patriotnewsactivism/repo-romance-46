...
interface Valuation {
  ...
  revenue_potential: { model: string; monthly_revenue_low: number; monthly_revenue_high: number; timeline: string };
...
async function fetchRepoMetrics(token: string, repo: string) {
  ...
  try {
    ...
    if (treeRes.ok) {
      ...
      hasTests = tree.tree?.some((t: { path: string }) => /test|spec|__tests__|\.test\.|\.spec\./i.test(t.path)) || false;
      hasCI = tree.tree?.some((t: { path: string }) => t.path.startsWith(".github/workflows/")) || false;
  ...
  let commitCount = 0;
  try {
    const commitRes = await fetch(`https://api.github.com/repos/${repo}/commits?per_page=100`, { headers });
    if (commitRes.ok) {
      ...
      responseFormat: {
        ...
        json_schema: {
          ...
          schema: {
            ...
            required: [
              "estimated_value_low", "estimated_value_high", "currency", "valuation_method",
              "confidence", "factors", "revenue_potential", "comparables", "risks", "upsides", "summary",
            ],
          },
        },
      },
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { data: analysis } = await req.supabase!.from("analyses").select("*").eq("id", id).eq("user_id", userId).maybeSingle();
    ...
    if (valueableRepos.size === 0) {
      throw Object.assign(new Error("No individual repos to value — only combine recommendations found."), { status: 400 });
    }
      ...
      try {
        ...
        const analysisContext = matchingItem
          ? {
              ...
              estimated_hours: (matchingItem as Record<string, unknown>).estimated_hours as number | null,
    ...
    if (valuations.length === 0) throw new Error(`All valuations failed: ${errors.join("; ")}`);
    ...
    const result: PortfolioValuation = {
      ...
      diversification_score: Math.min(
        10,
        Math.round((new Set(valuations.flatMap((v) => v.revenue_potential.model)).size / valuations.length) * 10),
      ),
      recommendation:
        totalHigh > 500000
    ...
    await req.supabase!.from("analyses").update({ valuation: result as unknown as never }).eq("id", id);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const { data: analysis } = await req.supabase!.from("analyses").select("valuation").eq("id", id).eq("user_id", req.userId!).maybeSingle();
    if (!analysis) throw Object.assign(new Error("Analysis not found"), { status: 404 });
    res.json((analysis as Record<string, unknown>).valuation || null);
  }),
);
