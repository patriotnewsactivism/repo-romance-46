...
import { IMMUTABLE_AGENT_SAFETY_POLICY, resolvePromptStrategy } from "../lib/prompt-strategy-evolution";
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { plan, planHash, reasoning } = await prepareFinishPlan(req.supabase!, userId, {
      repo: input.repo,
    ...
    const effectivePromptVersion = plan.reasoning?.promptVersion ?? promptStrategy.version;
    ...
    if (stepError) {
      await req.supabase!.from("completion_runs").delete().eq("id", run.id).eq("user_id", userId);
