...
} from "../lib/prompt-strategy-evolution";
import { selectSpecialists, type SpecialistSelection } from "../lib/specialist-agents";
...
function coreObjective(role: CoreAgentRole): string {
  if (role === "architect") {
    return "Define the smallest architecture-safe completion sequence. Identify interfaces that must remain stable and prerequisites that must land first. Use measured strategy history to avoid plans that previously produced weak completion deltas.";
  }
  if (role === "quality-security") {
    return "Act as a skeptical senior reviewer. Find failure modes, missing tests, auth/secret/permission hazards, migration risk, and required acceptance checks in the architect plan.";
  }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [repoSnapshot, learning, aiCredential, promptStrategy] = await Promise.all([
      ...
      resolvePromptStrategy(req.supabase!, userId),
    ]);
    ...
    const architect = await runReasoningAgent(
      "architect",
      coreObjective("architect"),
      sharedContext,
      ai,
      promptStrategy.guidance,
    );
    const qualitySecurity = await runReasoningAgent(
      "quality-security",
      coreObjective("quality-security"),
      { ...sharedContext, architect },
      ai,
      promptStrategy.guidance,
    );
    const productInvestment = await runReasoningAgent(
      "product-investment",
      coreObjective("product-investment"),
    ...
    const specialists = await Promise.all(
      specialistSelections.map((selection) =>
        runReasoningAgent(
          selection.role,
          selection.objective,
          { ...sharedContext, architect, qualitySecurity, productInvestment, specialistSelection: specialistTrace(selection) },
          ai,
          promptStrategy.guidance,
        ),
      ),
    ...
    const validationSteps = [qualitySecurity, ...specialists].flatMap((agent) =>
      agent.validation.map((step) => `Acceptance requirement (${agent.role}): ${step}`),
    );
    const specialistPriorities = specialists.flatMap((agent) => agent.priorities);
    const combinedNextSteps = uniqueSteps(
      input.nextSteps ?? [],
      architect.priorities,
      qualitySecurity.priorities,
      productInvestment.priorities,
      specialistPriorities,
      learnedGuidance,
      validationSteps,
    );
    ...
    const runInsert = await insertCompletionRunCompat(
      req.supabase!,
      {
        ...
        prompt_version: promptStrategy.version,
    ...
    const { error: eventError } = await req.supabase!.from("completion_events").insert({
      ...
      kind: "agent_council",
      status: "info",
      message: `Three core reasoning agents${specialists.length ? ` plus ${specialists.length} evidence-selected specialist${specialists.length === 1 ? "" : "s"}` : ""} produced a completion brief before the coding agent generated ${plan.changes.length} exact file changes.`,
      metadata: {
        promptVersion: promptStrategy.version,
        promptArm: promptStrategy.arm,
